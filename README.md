# 👩‍💻 Zeynep Üstün

Merhaba! Ben Zeynep Üstün.  
🎓 Ankara Yıldırım Beyazıt Üniversitesi Bilgisayar Mühendisliği 4. sınıf öğrencisiyim. 
Lisans eğitimimi %100 İngilizce alıyorum ve üniversite hayatım boyunca hem teknik hem sosyal yönümü geliştirmeye büyük önem veriyorum.

## 💼 Hakkımda

- Java, Python, C ve C++ dillerinde uygulamalar ve projeler geliştirdim.
- Web tasarımı, mobil uygulama geliştirme ve oyun geliştirme alanlarına ilgi duyuyorum.
- Tübitak-SAGE’de gömülü yazılım stajı yaparak C++ ile network programlama ve sensör verileri üzerine çalıştım.
- Aktif bir öğrenme süreci içindeyim; çeşitli gönüllü organizasyonlar ve teknoloji topluluklarında yer alıyorum.

## 🚀 Projelerimden Bazıları

- **🎯 Psikolog Web Platformu:** Kullanıcı profilleri, randevu takvimi ve terapi içerikleri içeren dinamik bir web sayfası.
  - Teknolojiler: HTML, CSS, JavaScript

- **🕹️ VR Korku Oyunu:** Unity ve VR teknolojileri ile geliştirilen, etkileşimli korku deneyimi sunan sanal gerçeklik oyunu.

- **📱 AR Mobil Oyun:** Vuforia ve Unity kullanarak geliştirilen artırılmış gerçeklikte balon patlatma oyunu.

- **✈️ Gerçek Zamanlı Uçak Takip Sistemi:** C++ ile geliştirilmiş, sensör verilerini işleyip analiz eden gömülü yazılım projesi. (TÜBİTAK SAGE)

- **🌐 Antideprem+:** TEKNOFEST kapsamında, afet yönetimi odaklı web projesi.  
  - Rolüm: Takım liderliği, proje planlama, sosyal farkındalık

- **🚁 BKVision:** Görüntü işleme ve otonom uçuş özellikli sınır güvenliği için geliştirilmiş drone yazılımı.

## 🧠 Yetenekler

- Programlama Dilleri: Java, Python, C, C++
- Geliştirme Alanları: Web Geliştirme, Mobil Geliştirme, Gömülü Sistemler, Oyun Geliştirme
- Araç ve Teknolojiler: Git, GitHub, Unity, HTML/CSS/JS
- Yetenekler: Takım Çalışması, Liderlik, Sosyal İletişim

## 🔗 Sosyal Medya & Bağlantılar

- [[LinkedIn](https://www.linkedin.com/in/zeynep-ustun)](https://www.linkedin.com/in/zeynep-ustun01/)
- [[Medium](https://medium.com/@zeyustun8)](https://medium.com/@zeyustun8)
- [[GitHub](https://github.com/ZeynepUstunn)](https://github.com/ZeynepUstunn)

💬 Yeni projelere ve iş birliklerine her zaman açığım. Profilimi ziyaret ettiğin için teşekkürler!

